...
export default function HomePage() {
  return (
    <main className="min-h-screen">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-20 sm:py-24 lg:py-28">
          <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-center">
            <div className="space-y-8">
              <div className="inline-flex items-center gap-2 rounded-full border border-cyan-500/20 bg-cyan-500/10 px-4 py-2 text-xs uppercase tracking-[0.24em] text-cyan-200 shadow-sm shadow-cyan-500/10">
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                <Link href="/courses/introduction/00-setup" className="inline-flex items-center justify-center rounded-full bg-cyan-500 px-8 py-3.5 text-sm font-semibold text-slate-950 shadow-lg shadow-cyan-500/20 transition hover:bg-cyan-400">
                  Start the course
                </Link>
                <Link href="/playground" className="inline-flex items-center justify-center rounded-full border border-slate-800 bg-slate-900/90 px-8 py-3.5 text-sm text-slate-100 transition hover:border-cyan-400 hover:text-white">
                  Open playground
                </Link>
              </div>
            </div>
            <div className="rounded-[2rem] border border-white/10 bg-slate-950/95 p-8 shadow-[0_32px_120px_rgba(15,23,42,0.35)] backdrop-blur-xl">
              <div className="space-y-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {['Live Examples', 'Visual Intuition', 'Code Playground', 'Quiz Feedback'].map((item) => (
                    <div key={item} className="rounded-3xl border border-slate-800 bg-slate-900/90 p-4">
                      ...
                      <p className="mt-2 text-sm text-slate-400">{item === 'Live Examples' ? 'Edit Python and see results instantly.' : item === 'Visual Intuition' ? 'Learn from diagrams and interactive math.' : item === 'Code Playground' ? 'Experiment with model code in the browser.' : 'Practice and verify concepts with quizzes.'}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {phases.map((phase) => (
            <Link
              ...
              className="group overflow-hidden rounded-[1.75rem] border border-slate-800 bg-slate-900/90 p-6 transition hover:-translate-y-1 hover:border-cyan-500/30 hover:bg-slate-900"
            >
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24 sm:pb-28">
        <div className="rounded-[2rem] border border-slate-800 bg-slate-950/90 p-10 shadow-[0_20px_120px_rgba(15,23,42,0.28)]">
          <div className="grid gap-6 lg:grid-cols-3">
            {['Live Python', 'Visual math', 'Auto-graded quizzes'].map((feature) => (
              <div key={feature} className="rounded-3xl border border-slate-800 bg-slate-900/90 p-6">
                ...
                <p className="mt-4 text-slate-300 leading-7 text-sm sm:text-base">{feature === 'Live Python' ? 'Run and edit NumPy code directly in your browser with no backend.' : feature === 'Visual math' ? 'See equations rendered clearly and learn from interactive visuals.' : 'Answer quiz questions as you learn and get instant feedback.'}</p>
              </div>
            ))}
          </div>
        </div>
